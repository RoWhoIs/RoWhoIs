...
def initialize(config, version: str, modded: bool):
    ...
    try:
        ...
        globProxies = typedefs.Proxies(config["Proxying"]["proxying_enabled"], config["Proxying"]["proxy_urls"], config["Proxying"]["username"], config["Proxying"]["password"])
    ...
    try:
        while globProxies.enabled:
            ...
            async with aiohttp.ClientSession() as session:
                if len(globProxies.ips) <= 0 and globProxies.logged: await logs.debug("No usable proxies found! Fallbacking to non-proxied.")
                else:
                    ...
                    if len(poolProxies.ips) <= 0 and globProxies.logged: await logs.debug("No usable proxies found! Fallbacking to non-proxied.")
                    elif globProxies.logged: await logs.debug(f"Refreshed proxy pool. {len(poolProxies.ips)} usable IP{'s' if len(poolProxies.ips) >= 2 else ''}.")
            await asyncio.sleep(300)
    except Exception as e:
        await logs.error(f"proxy_handler encountered a severe error while refreshing proxy pool: {e}")
        pass
...
async def proxy_picker(diderror: bool = False):
    ...
    try:
        ...
        if diderror and currentProxy.ip is not None:
            ...
            if globProxies.enabled: await logs.debug(f"Removing bad proxy {currentProxy.ip}.")
        ...
        await logs.error(f"Proxy picker fallbacking to non-proxied. Severe error: {e}")
        currentProxy = typedefs.Proxy(None)
        return
...
async def validate_cookie() -> None:
    ...
    async with aiohttp.ClientSession(cookies={".roblosecurity": BaseUserAuth.token}, headers={"User-Agent": uasString}) as main_session:
        async with main_session.get("https://users.roblox.com/v1/users/authenticated") as resp:
            ...
            else: await logs.error("Invalid ROBLOSECURITY cookie. RoWhoIs will not function properly.")
...
async def token_renewal(automated: bool = False) -> None:
    ...
    while True:
        try:
            ...
            await logs.error(f"token_renewal encountered an error while updating x-csrf-token: {e}")
            pass
        finally:
            if automated: break
    ...
    for retry in range(3):
        async with aiohttp.ClientSession(cookies={".roblosecurity": BaseUserAuth.token}, headers={"x-csrf-token": BaseUserAuth.csrf, 'User-Agent': uasString}) as main_session:
            try:
                ...
                if not productionMode: await logs.info(f"{logBlurb}", shard_id=shard_id) # PRIVACY FILTER
                try:
                    async with main_session.request(method.lower(), f"https://{node}.roblox.com/{endpoint}", timeout=4, proxy=currentProxy.ip, proxy_auth=globProxies.auth, **kwargs) as resp:
                        ...
                        await logs.warning(f"{logBlurb}: {resp.status} {('- ' + str(retry + 1) + '/3')}", shard_id=shard_id)
                    ...
                    await logs.error(f"{logBlurb}: {type(e)} |  {e if not isinstance(e, asyncio.exceptions.TimeoutError) else 'Timed out.'}", shard_id=shard_id)
            except Exception as e:
                await logs.error(f"{logBlurb}: Severe error: {type(e)} | {e}", shard_id=shard_id)
                raise errors.UnexpectedServerResponseError
    await logs.error(f"{logBlurb}: Failed after {retry + 1} attempt{'s' if retry >= 1 else ''}.", shard_id=shard_id)
...
async def GetFileContent(asset_id: int, version: int | None = None, shard_id: int | None = None) -> bytes:
    ...
    try:
        ...
        if not productionMode: await logs.info(logBlurb, shard_id=shard_id)
        async with aiohttp.ClientSession() as main_session:
            async with main_session.request("GET", f"https://assetdelivery.roblox.com/v1/asset/?id={asset_id}&version={version if version is not None else ''}", headers={'User-Agent': uasString}, proxy=currentProxy.ip, proxy_auth=globProxies.auth) as resp:
                if resp.status == 200:
                    ...
                    await logs.warning(f"{logBlurb}: {resp.status}", shard_id=shard_id)
                    raise errors.UnexpectedServerResponseError
        ...
        for retry in range(3):
            async with session.get("https://www.rolimons.com/itemapi/itemdetails", headers={'User-Agent': uasString}) as resp:
                if resp.status == 200: return await resp.json()
                    ...
                    await logs.warning(f"GET rolimons | itemdetails: {resp.status} (WAIT 5s) {retry + 1}/3")
                    await asyncio.sleep(5)
                else: await logs.warning(f"GET rolimons | itemdetails: {resp.status} {retry + 1}/3")
        await logs.error(f"GET rolimons | itemdetails: Failed after 3 attempts.")
        raise errors.UnexpectedServerResponseError
...
async def heartbeat() -> bool | None:
    ...
    try:
        ...
        await logs.warning(f"Heartbeat error: {e}")
        return False
