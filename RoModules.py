...
async def get_player_thumbnail(user_id:int, size):
    try:
        ...
        return "https://www.robloxians.com/resources/not-available.png"
...
async def get_socials(user_id:int): # Returns Friends, Followers, Following
    try:
        friend_count, following_count, follow_count = await asyncio.gather(
            Roquest.Roquest("GET", "friends", f"v1/users/{user_id}/friends/count"),
            Roquest.Roquest("GET", "friends", f"v1/users/{user_id}/followings/count"),
            Roquest.Roquest("GET", "friends", f"v1/users/{user_id}/followers/count")
        )
        if friend_count[0] in [-1, 403]: return -2, -2, -2 # Please optimize at some point, this seems bloated
